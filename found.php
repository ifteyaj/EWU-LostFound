    ...
    <div class="container" style="padding-top: 3rem; padding-bottom: 5rem;">
        <div class="section-header" style="display:flex; justify-content:space-between; align-items:flex-end; margin-bottom: 2rem;">
            <div>
                ...
                <p style="color: var(--text-grey); margin-top: 0.5rem;">Items found on campus waiting for claim.</p>
            </div>
                ...
                if ($result && $result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        ...
                        <div class="item-card glass-card">
                            ...
                            <span class="card-location">Location: <?php echo htmlspecialchars($row['found_location']); ?></span>
                            <span class="card-location" style="margin-top:0;">Date: <?php echo date('M d, Y', strtotime($row['date_found'])); ?></span>
                            
                            <div style="margin-top: 1rem;">
                                <a href="item.php?type=found&id=<?php echo $row['id']; ?>" class="btn-pill" style="font-size:0.8rem; padding:0.5rem 1rem;">Details</a>
                            </div>
                        </div>
                        <?php
                    }
                } else {
                    echo "<div class='glass-card' style='grid-column: 1/-1; text-align:center; padding: 4rem; color: var(--text-grey);'><h3>No found items reported yet.</h3></div>";
                }
            }
            ?>
        </div>
